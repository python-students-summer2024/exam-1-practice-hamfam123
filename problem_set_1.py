...
def bark_with_validation():
  ...
  """
  name = input("What is your dog's name?\n").strip()
  if name.isalpha() is False or len(name) < 2:
    print("Name error!")
    return
  age = input("What is your dog's age?\n").strip()
  if age.isnumeric() is False or (0 <= int(age) <= 15) is False:
    print("Age error!")
    return
  breed = input("What is your dog's breed?\n").strip().capitalize()
  if breed not in ["Schnauzer", "Terrier", "Poodle", "Mastiff"]:
    print("Breed error!")
    return
  print(f'{name.capitalize()}, the {age} year old {breed}, says, "Woof!"')
...
def respond_to_anything_but_nonsense():
  ...
  """
  sentence = input("Enter a sentence.\n").strip()
  if sentence.lower().find('nonsense') >= 0:
    print("")
  else:
    if sentence[-1] == ".":
      print("That's true.")
